...
### AIS monitoring locations by year (point stations)

*Non-point monitoring where the area monitored is represented by a shapefile in SWIMS and not by a single Lat/Lng coordinate, do not show up in the map below, but do appear in the interactive map above (when the shapefiles have been updated).*

```{r fig.height=7}
county_counts <- wi_counties %>%
  left_join({
    stn_pts_annual %>% st_set_geometry(NULL) %>% count(year, county_name)
  }) %>% drop_na(year)
