...
county_shapes <- wi_counties |>
  ...
  mutate(
    ...
    label = sprintf("<b>%s County</b> - %s", county, monitored) |>
      lapply(shiny::HTML)
  )
...
leaflet() |>
  ...
  addPolygons(
    ...
    weight = .5,
    opacity = .5,
    fillColor = ~ colorNumeric("viridis", species)(species),
    fillOpacity = .1,
    group = "Counties"
  ) |>
  addPolygons(
  ...
  addCircleMarkers(
    data = ais_pts_labelled |>
      filter(n_species == 0),
    label = ~label,
    popup = ~popup,
    radius = 4,
    color = ~ if_else(year == max(year), "black", "gray"),
    weight = 1,
    opacity = 1,
    fillColor = ~ if_else(year == max(year), "cornflowerblue", "skyblue"),
    fillOpacity = 1
  ) |>
  addCircleMarkers(
    data = ais_pts_labelled |>
    ...
    weight = 1,
    opacity = 1,
    fillColor = ~ if_else(year == max(year), "darkorange", "orange"),
    fillOpacity = 1
  ) |>
