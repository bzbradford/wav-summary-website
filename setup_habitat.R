...
results <- readxl::read_excel("data_swims/wav_habitat.xlsx") |>
  clean_names() |>
  select(
...
# ip_groups
people <- read_excel("data_swims/wav_all_ip_groups.xlsx") |>
  clean_names() |>
  select(
    gsn = group_seq_no,
    last_name,
...
stns <- read_excel("data_swims/wav_all_stns.xlsx") |>
  clean_names() |>
  select(
    msn = monit_station_seq_no,
    station_id,
...
hab_score_names <- read_csv(
  "data_habitat/habitat-assessment-score-reference.csv",
...
fw_points <- fieldwork_info |>
  ...
  mutate(
    ...
    popup = create_popup(
      pick(everything()),
      "<b>==== Habitat assessment ====</b>"
    ),
    label = glue::glue(
      "<b>{date}</b> <i>{fieldwork_comment}</i>",
      .na = "No comment"
    ),
