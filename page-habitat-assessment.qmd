...
hab_annual_counts %>%
  ...
  geom_text(
    aes(label = sprintf("%s stations\n%s events", n_stations, n_events)),
    vjust = -0.5,
    size = 3
  ) +
...
fieldwork_info %>%
  ...
  labs(
    y = "Percent of stations assessed n times",
    x = NULL,
    fill = "Habitat assessments per year"
  ) +
  plt_theme +
...
fieldwork_info %>%
  ...
  left_join(
    count(fieldwork_info, year, name = "Total assessments"),
    by = "year"
  ) %>%
  rename(Year = year) %>%
  knitr::kable(align = "c")
...
```{r}
#| fig-height: 4
add_rect <- function(min, max, label, color, width = 5) {
  list(
    annotate(
      "rect",
      xmin = min,
...
hab_data_less10_complete %>%
  ...
  mutate(
    ...
    left = as.numeric(str_split_i(bin, ",", 1)),
    right = as.numeric(str_split_i(bin, ",", 2)),
    middle = (left + right) / 2
  ) %>%
  ggplot() +
  add_rect(0, 25, "Poor", alpha("red", .1)) +
  add_rect(25, 50, "Fair", alpha("orange", .1)) +
  add_rect(50, 75, "Good", alpha("blue", .1)) +
  add_rect(75, 100, "Excellent", alpha("green", .1)) +
  geom_col(
    aes(x = middle, y = n, fill = n),
    width = 5,
    color = "black",
    linewidth = .25
  ) +
  scale_fill_viridis_c() +
  scale_x_continuous(breaks = seq(0, 100, 10), expand = expansion(c(0, .01))) +
...
map_counties <- wi_counties |>
  left_join(county_totals) |>
  mutate(
    across(c(n_stations, n_events), ~ coalesce(.x, 0L)),
    tooltip_html = paste(
      sprintf("<b>%s County</b>", county_name),
      sprintf("<i>%s</i>", dnr_region),
      sprintf("%s stations", n_stations),
      sprintf("%s habitat assessments", n_events),
      sep = "<br>"
    )
  )
...
maplibre(
  ...
  fit_bounds(map_counties, padding = 20) |>
  # County Polygons
  add_fill_layer(
    id = "Counties",
    source = map_counties,
    fill_color = interpolate(
      column = "n_events",
      values = c(0, max(map_counties$n_events, na.rm = TRUE)),
      stops = c("#ffffb2", "#bd0026"), # YlOrRd palette endpoints
  ...
  # Habitat Assessment Events (clustered)
  add_circle_layer(
    id = "Assessments",
    source = fw_points,
    circle_radius = 5,
  ...
  add_layers_control(
    layers = c("Counties", "Stations", "Assessments"),
    position = "top-right"
  ) |>
...
*Click on a cluster to zoom in, click on any blue circle to view a record of a habitat assessment performed at that location. Green circles show DNR station locations, blue circles show individual fieldwork events. Counties are colored by the total number of habitat assessments performed in that county.*
