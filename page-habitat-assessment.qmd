...
add_rect <- function(min, max, label, color, width = 5) {
  list(
    ...
    annotate(
      ...
      x = (min + max) / 2,
      y = Inf,
      label = sprintf("%s\n(%s-%s)", label, min, if_else(max == 100, 100, max - 1)),
      vjust = 1.25,
      hjust = 0.5
    )
  )
...
hab_data_less10_complete %>%
  ...
  mutate(bin = cut(result_value, seq(0, 100, 5), include.lowest = TRUE, right = FALSE, ordered_result = TRUE)) %>%
  ...
  geom_col(aes(x = middle, y = n, fill = n), width = 5, color = "black", linewidth = .25) +
  scale_fill_viridis_c() +
