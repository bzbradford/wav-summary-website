...
*This map shows all point stations and river segments that were monitored at least once for AIS between 2008 and present. River segments are shown as red lines and point stations are shown as orange circles. Individual fieldwork events are shown as blue circles, or as numbered clusters when zoomed out. Click on a blue circle to get more information about an AIS fieldwork event.*
...
counties_with_results <- wi_counties %>%
  ...
  mutate(
    popup = create_popups(.),
    tooltip_html = if_else(
      !is.na(species),
      sprintf("<b>%s County</b> - %s species found", county_name, species),
      sprintf("<b>%s County</b> - Not monitored", county_name)
    )
  )

stn_lines_labelled <- stn_lines |>
  mutate(tooltip_html = paste(station_id, station_name))

stn_pts_labelled <- stn_pts |>
  mutate(tooltip_html = paste(station_id, station_name))

fw_pts_labelled <- all_pts |>
  mutate(
    tooltip_html = paste0(
      "<b>Station ", station_id, ": ", station_name, "</b><br>",
      "Fieldwork date: ", date,
      ", Waterbody monitored: ", `Waterbody Name`
    )
  )
...
maplibre(
  ...
  fit_bounds(all_pts, padding = 20) |>
  # County Polygons
  add_fill_layer(
    id = "Counties",
    source = counties_with_results,
    fill_color = interpolate(
      column = "species",
      values = c(
        min(counties_with_results$species, na.rm = TRUE),
        max(counties_with_results$species, na.rm = TRUE)
      ),
  ...
  # Fieldwork Events (clustered)
  add_circle_layer(
    id = "Fieldwork events",
    source = fw_pts_labelled,
    circle_radius = 5,
